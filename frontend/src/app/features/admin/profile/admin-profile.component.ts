...
@Component({
  ...
  template: `
    <div class="admin-profile app-page">
      <header class="page-head">
        <div>
          <h1>My Profile</h1>
          <p>Your admin account — same as passengers and riders: name, phone, and photo.</p>
        </div>
      </header>
  ...
  `,
  styles: [`
    .admin-profile { animation: fadeIn 0.45s ease-out; }
    .page-head {
      margin-bottom: 28px;
      h1 { font-size: 22px; font-weight: 700; color: var(--clr-text); }
      p { color: var(--clr-text-muted); font-size: 14px; margin-top: 4px; max-width: 48ch; line-height: 1.45; }
    }
    .profile-grid {
      display: grid;
